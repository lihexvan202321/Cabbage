...
function handleCustomerDeal(customer, accept, freeGiveaway = false) {
    if (freeGiveaway) {
        ...
        addLog(`免费赠送${giveawayAmount}个模拟器给美食博主,声誉+15`, 'positive');
    } else if (accept) {
        // 正常交易
...
function joinEvent(day) {
    ...
    if (eventConfig.type === 'quality') {
        ...
        if (won) {
            ...
            addLog('创意设计大赛获胜! 声誉+25, 解锁新菜品(永久销量+20%)', 'positive');
        } else {
            // 失败也给少量声誉
        ...
        <div class="modal-content" style="max-width: 700px;">
            ...
            <div style="display: grid; gap: 15px;">
                ...
                <div>
                    ...
                    <input type="text" id="event-name-input" placeholder="例如：春季模拟器大赛" 
...
function unlockAcademyFromUpgrade() {
    ...
    addLog('🎓 成功解锁模拟器神学院！', 'positive');
    addLog('✨ 销售中心已新增"模拟器神学院"子页面', 'positive');
    updateUI();
    saveGame();
}
...
function checkRestaurantUpgrade() {
    ...
    if (GameState.day >= CONFIG.RESTAURANT_UPGRADE_DAY &&
        ...
        addLog('🎉 恭喜!你现在可以升级为模拟器餐馆了!', 'positive');
        updateUI();
    }
}
...
function upgradeToRestaurant() {
    ...
    addLog('🏪 成功升级为模拟器餐馆! 成本价和最高定价已提升!', 'positive');
    updateUI();
    saveGame();
}

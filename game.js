...
const CONFIG = {
    ...
    // 实验室配置
    LAB_UPGRADE_COST: 1000000000,  // 升级为实验室的费用(10亿)
        ...
        if (cultureCooldownEl) {
            ...
            if (lastDay > 0) {
                ...
                if (daysSince < 10) {
                    ...
                    cultureCooldownEl.textContent = `🕒 冷却中: 还需 ${remaining} 天 (第${lastDay + 10}天可用)`;
...
function executeExperiment() {
    ...
    const amount = GameState.labProduction.experimentAmount;
    
    if (GameState.inventory < amount) {
        alert(`库存不足!需要 ${formatNumber(amount)} 颗卷心菜进行实验`);
        return;
    }

    // 计算销量提升百分比
    const currentLevel = GameState.labProduction.experimentLevel;
    const newLevel = currentLevel + amount;
    const currentBonus = (currentLevel / 1000 * 100).toFixed(1);
    const newBonus = (newLevel / 1000 * 100).toFixed(1);

    const confirm = window.confirm(
        `确定要进行实验研究吗?\n\n` +
        `• 消耗: ${formatNumber(amount)} 颗卷心菜\n` +
        `• 当前实验等级: ${formatNumber(currentLevel)} (销量+${currentBonus}%)\n` +
        `• 实验后等级: ${formatNumber(newLevel)} (销量+${newBonus}%)\n` +
        `• 销量提升: +${newBonus - currentBonus}%\n`
    );
    ...
    GameState.labProduction.experimentLevel += amount;
    
    addLog(`🔬 实验研究完成! 消耗 ${formatNumber(amount)} 颗卷心菜,实验等级提升至 ${formatNumber(newLevel)} (销量+${newBonus}%)`, 'positive');
    updateUI();
    saveGame();
}
...
function executeCulture() {
    ...
    if (GameState.labProduction.lastCultureDay > 0 && daysSinceLastCulture < 10) {
        const remainingDays = 10 - daysSinceLastCulture;
    ...
    addLog(`🌱 组织培养完成! 消耗 ${formatNumber(costCabbage)} 颗卷心菜和 ${formatMoney(costMoney)},获得 ${formatNumber(gain)} 颗卷心菜 (下次可用: 第${GameState.day + 10}天)`, 'positive');
    updateUI();
    saveGame();
}

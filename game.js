...
const CONFIG = {
    ...
    // 营销配置
    MARKETING_OPTIONS: {
        flyer: { cost: 200, salesBonus: 0.15, reputationBonus: 0, duration: 3, name: '传单派发' },
        social: { cost: 500, salesBonus: 0.30, reputationBonus: 5, duration: 5, name: '社交媒体' },
        radio: { cost: 1000, salesBonus: 0.50, reputationBonus: 10, duration: 7, name: '本地电台' },
        tv: { cost: 3000, salesBonus: 1.0, reputationBonus: 20, duration: 10, name: '电视广告' }
    },
...
function updateUI() {
    ...
    // 更新进度条
    const progressPercent = (GameState.totalSold / CONFIG.TARGET_SALES) * 100;
...
function nextDay() {
    ...
    // 如果今天还没销售,先执行销售
    if (!GameState.todaySales.hasSold && GameState.inventory > 0) {
        executeSales();
    }
...
function checkGameEnd() {
    ...
    // 胜利条件: 售完所有卷心菜
    if (GameState.inventory <= 0 || GameState.totalSold >= CONFIG.TARGET_SALES) {
        endGame(true);
        return;
    }
...
function endGame(won) {
    ...
    if (won) {
        ...
        let rating = 'C';
        if (daysUsed <= 30) rating = 'S';
        else if (daysUsed <= 45) rating = 'A';
        else if (daysUsed <= 60) rating = 'B';
        ...
        body.innerHTML = `
            <div class="game-over-result">
                ...
                <div style="font-size: 2rem; margin: 20px 0;">
                    评级: <strong style="color: var(--primary); font-size: 3rem;">${rating}</strong>
                </div>
            </div>
        `;
    } else {
...
function resetGameState() {
    ...
    // 重置价格滑块
    document.getElementById('price-slider').value = CONFIG.BASE_PRICE;
    document.getElementById('price-display').textContent = CONFIG.BASE_PRICE;

    // 重置促销选项
    document.getElementById('promo-bogo').checked = false;
    document.getElementById('promo-discount').checked = false;
    document.getElementById('promo-bundle').checked = false;
